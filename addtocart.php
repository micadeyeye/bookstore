...
<table width="741" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td height="444" valign="top" bgcolor="#CCCCCC"><form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" enctype="multipart/form-data">
     <?php
	  if(isset($_GET['booksnumber'])){require "showitem_class.php";}
	 elseif((isset($_POST['additem']))&&($chkmemberstatus->get('email'))){require "excart.php";}
	 elseif(((isset($_GET['delete']))&&($_GET['delete']=='YES'))&&($chkmemberstatus->get('email'))){require "excart.php";}
	 	  elseif(isset($_POST['calculate'])){require "excart.php";}
	  else{echo "<center>Welcome Guest,<hr class='dashline' width='100%' size='1'><p><br>Already a member, then <a href='login.php'><strong class='boldlink'>log in</strong></a> to see or update your cart.<p>If you are not a member, <a href='account.php'><strong class='boldlink'>click here</strong></a> to register(free) <br>inorder to fully use this online store<h4 align='center'>Thanks,</h4></center>";}
	 ?>
	    </form></td>
  </tr>
  <tr>
